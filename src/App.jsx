          ...
          <Card className="border-white/10 bg-white/[0.04] text-white">
            <CardContent className="p-7">
              ...
              <ul className="mt-5 space-y-4 text-slate-300">
                {["The service you want: Personal, Content, or Executive assistant", "Top 1–3 workflows you want built first", "Tools you want connected, such as Gmail, Calendar, Notion, Sheets, Discord, or Telegram", "Whether you want hosting and monthly support", "Any examples, files, notes, or current workflow screenshots"].map((item) => <li key={item} className="flex gap-3"><CheckCircle2 className="mt-1 h-5 w-5 text-cyan-300" /><span>{item}</span></li>)}
              </ul>
              <div className="mt-7 flex flex-col gap-3">
                <Button asChild>
                  <a href={BRAND.calendlyUrl} target="_blank" rel="noreferrer">Open Calendly booking</a>
                </Button>
                <Button onClick={() => setShowEmbed(!showEmbed)} variant="outline">
    ...
    <section className="bg-slate-950 px-5 py-20">
      <div className="mx-auto max-w-5xl rounded-[2rem] border border-white/10 bg-gradient-to-r from-cyan-400/15 to-violet-500/15 p-8 text-center md:p-12">
        ...
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <Button asChild size="lg"><a href="#book">Book a consultation</a></Button>
          <Button asChild size="lg" variant="outline"><a href={`mailto:${BRAND.contactEmail}`}>Email SmartWorkHub</a></Button>
        </div>
      </div>
    </section>
  );
